...
- **YAML front matter** — 文档开头的 `--- ... ---` 元信息块渲染为预览顶部可折叠的元信息卡片（YAML 语法高亮，导出 HTML 同样保留），编辑器内也按 YAML 高亮
...
- **系统文件关联** — 安装后（macOS `.app` / Windows 安装包）`.md` / `.markdown` 的「打开方式」中出现 markup，双击即用 markup 打开（单实例：再双击会交给已运行的窗口），并可在系统设置中设为默认 Markdown 应用
